...
const AUDIO_CHECK_PS1 = join(__dirname, "tools", "AudioCheck.ps1");
const IDLE_THRESHOLD_SECONDS = 120; // 2 minutes
const MEDIA_CHECK_INTERVAL_MS = 2000; // Check audio output every 2 seconds
...
const MEDIA_GRACE_PERIOD_MS = 30000; // 30 seconds grace after media stops
...
function checkMediaPlaying() {
  return new Promise((resolve) => {
    // Uses AudioCheck.ps1 which detects actual audio output (works with any app)
    execFile(
      "powershell",
      ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", AUDIO_CHECK_PS1],
      { timeout: 3000 },
      (error, stdout) => {
        if (error) {
          resolve(false);
...
// Periodically update media playing state (non-blocking, uses cached value)
async function updateMediaState() {
  const now = Date.now();
  // Only start a new check if enough time has passed since last check
  if (now - lastMediaCheck > MEDIA_CHECK_INTERVAL_MS) {
    lastMediaCheck = now;
    // Start the check in background, don't await - use cached value for this cycle
    checkMediaPlaying().then(result => {
      isMediaPlaying = result;
    }).catch(() => {
      isMediaPlaying = false;
    });
  }
  return isMediaPlaying; // Return cached value immediately
}
...
function getSessionKey(win) {
  ...
  const isBrowser = /chrome|chromium|firefox|edge|brave|opera|safari/i.test(
    app,
  );
...
async function checkWindow() {
  try {
    // 1. Get Idle Time & Active Window
    const [idleSeconds, win] = await Promise.all([
      getIdleTimeSeconds(),
      activeWin().catch(() => null)
    ]);

    // 2. Check if media (audio) is playing - this now uses AudioCheck.ps1 which
    // detects actual system audio output, working with ANY app (mpv, VLC, browsers, etc.)
    let mediaPlaying = await updateMediaState();

    if (mediaPlaying) {
      lastMediaPlayingTime = Date.now();
    }

    // 3. Check if gamepad has activity (skip idle detection if gamepad is active)
    const gamepadActive = await updateGamepadState();

    // Only consider idle if: idle time exceeded AND no media playing AND no gamepad
    const isMediaGraceActive =
      Date.now() - lastMediaPlayingTime < MEDIA_GRACE_PERIOD_MS;
    ...
    const isBrowser = BROWSER_REGEX.test(win.owner.name);
